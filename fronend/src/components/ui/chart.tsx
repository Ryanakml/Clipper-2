...
const ChartStyle = ({ id, config }: { id: string; config: ChartConfig }) => {
  const colorConfig = Object.entries(config).filter(([, config]) =>
    Boolean(config.theme ?? config.color),
  );
...
function ChartTooltipContent({
  ...
  const payloadItems = React.useMemo(() => payload ?? [], [payload]);

  const tooltipLabel = React.useMemo(() => {
    if (hideLabel || !payloadItems.length) {
      return null;
    }
      ...
      <div className="grid gap-1.5">
        {payloadItems
          ...
          .map((item, index) => {
            ...
            const itemConfig = getPayloadConfigFromPayload(config, item, key);
            const payloadFill = getStringValue(item.payload, "fill");
            ...
            return (
              ...
              >
                {formatter && hasValue && hasName ? (
                  formatter(itemValue, itemName, item, index, payloadItems)
                ) : (
                  <>
                    {itemConfig?.icon ? (
                      <itemConfig.icon />
...
function getPayloadConfigFromPayload(
  ...
  const payloadValue = payloadRecord[key];
