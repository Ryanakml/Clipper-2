...
const DashboardClient = ({
  ...
  // Keep local list in sync with server data while preserving in-flight uploads
  // that have not reached the server yet.
  useEffect(() => {
    setUploads((prev) => {
      const incomingIds = new Set(uploadedFiles.map((item) => item.id));
      const pendingUploads = prev.filter(
        (item) => item.status === "uploading" && !incomingIds.has(item.id),
      );
      return [...uploadedFiles, ...pendingUploads];
    });
  }, [uploadedFiles]);
  ...
  return (
    <div className="mx-auto flex max-w-5xl flex-col space-y-6 px-4 py-8">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-semibold tracking-tight">Clip Now</h1>
